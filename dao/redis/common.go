...
func FavoriteMoveTomysql() error {
	...
	for _, key := range keys {
		...
		// 检查是否存在对应ID
		v, err := mysql.GetVideoById(ctx, videoID)
		if err != nil {
			logger.Errorln(err.Error())
			return err
		}
		...
		// 查询是否存在点赞记录
		favorite, err := mysql.GetFavoriteVideoRelationByUserVideoID(ctx, userID, videoID)
		if err != nil {
			logger.Errorln(err.Error())
			return err
		} else if favorite == nil && redisAt == "1" {
			// 数据库中没有该点赞记录，且最终状态为点赞，则插入数据库
			video, err := mysql.GetVideoById(ctx, videoID)
			if err != nil {
				logger.Errorln(err.Error())
				return err
			}
			err = mysql.CreateVideoFavorite(ctx, userID, videoID, int64(video.AuthorID))
			// 插入后，删除Redis中对应记录
			delErr := deleteKeys(ctx, key, FavoriteMutex)
			if delErr != nil {
				logger.Errorln(delErr.Error())
			...
			err = mysql.DelFavoriteByUserVideoID(ctx, userID, videoID, int64(video.AuthorID))
			// 插入后，删除Redis中对应记录
			delErr := deleteKeys(ctx, key, FavoriteMutex)
			if delErr != nil {
				logger.Errorln(delErr.Error())
...
func RelationMoveTomysql() error {
	...
	for _, key := range keys {
		...
		tu, err := mysql.GetUserByID(ctx, toUserID)
		if err != nil {
			logger.Errorln(err.Error())
			return err
		}
		...
		if err != nil {
			...
			err = mysql.DelRelationByUserIDs(ctx, userID, toUserID)
			// 删除Redis中对应记录
			delErr := deleteKeys(ctx, key, RelationMutex)
			if delErr != nil {
				logger.Errorln(delErr.Error())
...
func GoCronFavorite() {
	...
	s.Every(frequency).Tag("favoriteRedis").Seconds().Do(FavoriteMoveTomysql)
	s.StartAsync()
}

func GoCronRelation() {
	s := gocron.NewSchedule()
	s.Every(frequency).Tag("relationRedis").Seconds().Do(RelationMoveTomysql)
	s.StartAsync()
}
